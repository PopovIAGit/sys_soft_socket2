...
void SetCoilPWMFreq(uint16_t freq)
{ /*in Hz*/
  if (freq > 100)
    freq = 100;
...
uint32_t GetCoilPWMFreq()
{
  ...
  if (htim1.Instance->ARR != 0)
    return Freq;
  return 0;
}

void SetCoilCurrent(uint16_t i)
{ /*in mkA*/
  uint64_t u;
  uint16_t n;
  uint8_t byte;
...
void SetTestVolatge(uint16_t v)
{ /*in mV*/
  uint32_t n;
...
{ /*in MOmh*/
  ...
  U_MR = (uint32_t)U_MR_avg * UREF / 4096;
  if ((U_MR < TEST_VLT) && (U_MR != 0))
    R_ISO = (TEST_VLT - U_MR) * R_izm * 100 / U_MR; // in MOmh
  else
    R_ISO = 100;
  if (R_ISO > 32000)
    R_ISO = 100;
    ...
    if (evt.status == osEventMail)
    {
      ...
      for (idx = 0; idx < BUFSIZE / 4 - 50; idx++)
      {
        ...
        for (idy = 0; idy < 50; idy++)
          diffRS_avg = diffRS_avg + diff_RS[idx + idy];
      ...
      if (dev_cntrl.activate_rstest)
      {
        ...
        if (R_RSon_avg < dev_cntrl.max_Rs)
          dispertion.good_cnt++;
        dispertion.cnt++;
        dev_cntrl.cnt1k++;
